...
class TestAnnotations(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # We're going to be messing with this mock, so it needs to be reset for each test.
        bpy.data.texts = unittest.mock.MagicMock()
    ...
    def xml_to_filestream(self, root, path):
        ...
        :param path: The path to the file within the 3MF archive that we must mock.
    ...
    def test_add_rels_empty(self):
        ...
        self.assertDictEqual(
            ...
            {},
            "The relationships file was empty, so there should not be any annotations yet.")
    ...
    def test_add_rels_duplicates(self):
        ...
        self.annotations.add_rels(rels_file)
        rels_file.seek(0)
        # Also add the same file twice, to test removal of duplicates over multiple files.
        self.annotations.add_rels(rels_file)
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            expected_annotations,
            "Even though the relationship was added 8 times, there may only be one resulting relationship.")
    ...
    def test_add_rels_missing_attributes(self):
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            {},
            "Both relationships were broken, so they should not get stored.")
    ...
    def test_add_rels_base_path(self):
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            expected_annotations,
            "The source of the annotation is the metadata directory.")
    ...
    def test_add_content_types_unknown(self):
        ...
        This is the happy path for adding content types, since only the unknown content types get stored.
        """
        file_stream = io.BytesIO()
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            expected_annotations,
            "There was a content type specified, so we should store that.")
    ...
    def test_add_content_types_unspecified(self):
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            {},
            "The file had no specified content type, so we shouldn't store that.")

    def test_add_content_types_known(self):
        """
        Tests adding files with a content type that the 3MF format add-on will write.

        We shouldn't store those annotations since they can change since this add-on decides for itself where the files
        are saved.
        """
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            {},
            "Both files had content types that we already know, so those shouldn't get stored.")
    ...
    def test_add_content_types_conflict(self):
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            expected_annotations,
            "The same file had multiple content types, so it is now in conflict.")
        ...
        self.assertDictEqual(
            self.annotations.annotations,
            expected_annotations,
            "Adding a content type again should still let it be in conflict.")
    ...
    def test_write_rels_different_annotations(self):
        """
        Test writing relationships when there are only different annotations stored.
        ...
        # Add an annotation that is not a Relationship.
        self.annotations.annotations["file.txt"] = {io_mesh_3mf.annotations.ContentType(mime_type="mim")}
        self.annotations.write_rels(archive)
        ...
        self.assertEqual(
            len(relationships),
            1,
            "There must still only be the default 3D model relationship. No Content Type relationship or anything.")
    ...
    def test_write_rels_relationship(self):
        ...
        # Add a relationship to write. Source is the root.
        self.annotations.annotations["file.txt"] = {io_mesh_3mf.annotations.Relationship(namespace="nsp", source="/")}
        self.annotations.write_rels(archive)
    ...
    def test_write_content_types_empty(self):
        """
        Tests writing a content types file where there are no special content types.
    ...
    def test_write_content_types_single(self):
        ...
        self.assertEqual(
            len(my_default),
            1,
            "Since there is only 1 file with a .txt extension, that content type should be selected as the default.")
        self.assertEqual(
            my_default[0].attrib["ContentType"],
            "some MIME type",
            "This was the content type that we added for that file.")

    def test_write_content_types_same_mime(self):
        """
        Test writing content types when there are multiple annotated files with the same content type.
        """
        archive = unittest.mock.MagicMock()
        file = io.BytesIO()  # Simulate the [Content_Types].xml file.
        file.close = lambda: None  # Don't close this please.
    ...
    def test_write_content_types_different_mime(self):
        """
        Test writing content types when there are multiple annotated files with different content types.
        """
        archive = unittest.mock.MagicMock()
        file = io.BytesIO()  # Simulate the [Content_Types].xml file.
        file.close = lambda: None  # Don't close this please.
        ...
        # Create 2 files with the same extension and MIME type, which will be the default MIME type since it's more
        # common.
        for i in range(2):
            mock_file = io.BytesIO()
            mock_file.name = f"path/to/file{i}.txt"
        ...
        self.assertEqual(
            ...
            "/path/to/unique_file.txt",
            "The override points to the file with a unique MIME type.")
        self.assertEqual(
            unique_override[0].attrib["ContentType"],
            "unique",
            "The override specifies the unique MIME type which is different from the most common one.")
    ...
    def test_store_empty(self):
        ...
        # The JSON dump is empty since there is no annotation.
        bpy.data.texts.new().write.assert_called_once_with(json.dumps({}))

    def test_store_relationship(self):
        """
        Test storing relationship annotations.
        """
        self.annotations.annotations["some/file.txt"] = {
            io_mesh_3mf.annotations.Relationship(namespace="nsp", source="src")
        }
        self.annotations.store()
        ...
        # There must be a relationship in the JSON dump of this instance.
        bpy.data.texts.new().write.assert_called_once_with(json.dumps(ground_truth))
    ...
    def test_store_content_type(self):
        ...
        # There must be a content type in the JSON dump of this instance.
        bpy.data.texts.new().write.assert_called_once_with(json.dumps(ground_truth))
    ...
    def test_store_content_type_conflict(self):
        ...
        # There must be a marker in the JSON dump to indicate the content type conflict.
        bpy.data.texts.new().write.assert_called_once_with(json.dumps(ground_truth))
    ...
    def test_retrieve_empty(self):
        ...
        Well, the file is not completely empty since it's correctly formatted JSON. But it contains no annotations.
        """
        contents = json.dumps({})
